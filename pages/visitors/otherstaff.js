...
import CheckinOtherStaffModal from "../../components/staff/checkin-otherstaff-modal";
...
export default function Visitors() {
  ...
  const visitors = items?.lists?.nonMenziesvisitors;
  //Get Dashboard Stats
  ...
  return (
    <PageContainer>
      {/* breadcrumb */}
      <Breadcrumb title="Other Staff" items={BCrumb} />
      {/* end breadcrumb */}
      <Box>
        ...
        <div className="w-full flex flex-wrap mt-2">
          ...
          <DashboardCard title="Total Staff" value={dashboard?.non_menziesvisitors ?? 0} color="orange" />
          <DashboardCard title="Total Visits" value={dashboard?.non_menzies_visits ?? 0} color="red" />
          <DashboardCard title="Total Check In" value={dashboard?.non_menzies_visits_in ?? 0} color="blue" />
          <DashboardCard title="Total Check Out" value={dashboard?.non_menzies_visits_out ?? 0} color="purple" />
        </div>
        </div>
          ...
          <Card>
            ...
            <Table>
              ...
              <tbody>
                {!isLoading &&
                  visitors &&
                  visitors?.map((item) => ( //After pagination use this
                  // visitors?.map((item) => (
                    <tr key={item?.id} className="border-b" >
                      ...
                      <td className="py-0 pl-14 2xl:pl-4">
                        <span className="flex justify-end items-center w-full gap-2">
                          ...
                          <CheckinOtherStaffModal item={item} />
