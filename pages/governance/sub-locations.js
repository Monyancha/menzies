...
export default function SubLocations() {
  ...
  return (
    <PageContainer>
      {/* breadcrumb */}
      <Breadcrumb title="Sub-Locations" items={BCrumb} />
      {/* end breadcrumb */}
      <Box>
        <header className="flex flex-wrap justify-between items-end w-full">
          ...
          <div className="flex space-x-2 w-full mt-3 md:mt-0 md:w-6/12 justify-center md:justify-end flex-wrap">
            {/* <NewConsignmentModal /> */}
            <Button variant="outline" leftIcon={<IconPlus size={18} />}>New Sub-Location</Button>
          </div>
        </header>
        ...
        <div className="w-full flex flex-wrap mt-2">
          <Card>
            ...
            <Table>
              ...
              {Object.entries(wards).map(([county, subWards]) => (
                    subWards.map((subWard, index) => (
                    <tr className="border-b" key={`${county}-${subWard}`}>
                        <td>{index + 1}</td>
                        <td>{county}</td>
                        <td>{subWard}</td>
                        <td>Sub-Location Name</td>
                        <td className="text-right">
                        19-02-2024 {/* Replace with actual date */}
                        </td>
                        <td className="text-right">
