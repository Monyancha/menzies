...
export default function Acceptance() {
  ...
  const itemStatus = useSelector(
    (state) => state.cargo.getAcceptanceStatus
  );
  const items = useSelector((state) => state.cargo.getAcceptance);
  ...
  console.log('Items data mmm', items);
  ...
  return (
    <PageContainer>
      {/* breadcrumb */}
      <Breadcrumb title="Awaiting Acceptance" items={BCrumb} />
      {/* end breadcrumb */}
      <Box>
          ...
          <Card>
            ...
            <Table>
              ...
              <tbody>
                {!isLoading &&
                  items &&
                  // items?.data?.data?.map((item) => ( //After pagination use this
                  items?.data?.map((item) => (
