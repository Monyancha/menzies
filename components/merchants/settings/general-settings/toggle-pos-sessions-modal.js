import {
  ...
  export default function TogglePosSessionModal({
    opened = null,
    setOpened = () => {},
  } = {}) {
    ...
    return (
      <>
        <Modal
          opened={opened}
          title="Enable/Disable POS Sessions"
          ...
          <div className="flex flex-col w-full mb-4 space-y-2">
            ...
            {!isLoadingMerchantFlags && (
              <Formik
                initialValues={{
                  enabled:
                    merchantFlags?.find((item) => item.name === "show-pos-sessions")
                      ?.details?.value ?? false,
                }}
                ...
                onSubmit={async (values) => {
                  ...
                  const body = { value: values.enabled, name: "show-pos-sessions" };
