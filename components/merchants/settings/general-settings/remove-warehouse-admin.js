...
import { removeWarehouseAdmin,getExistingWarehouseStaffs } from "@/store/merchants/settings/access-control-slice";
import { useDispatch, useSelector } from "react-redux";
import { showNotification } from "@mantine/notifications";

function RemoveWarehouseAdmin({ item }) {
  ...
  async function submitDetails() {
    ...
    params["itemId"] = item.staff_id;
    params["warehouse_id"] = item.warehouse_id;

    try {
      dispatch(removeWarehouseAdmin(params));
      ...
      setOpened(false);
      dispatch(getExistingWarehouseStaffs(params));
    } catch (e) {
      let message = null;
      if (e?.message ?? null) {
        message = e.message;
...
export default RemoveWarehouseAdmin;
