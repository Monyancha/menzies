...
import { getProductVariations } from "@/store/merchants/reports/reports-slice";
...
function ProductVariations() {
  ...
  const inventory = useSelector(
    (state) => state.reports.product_variations_list
  );
  ...
  const exportPDF = async () => {
    ...
    const endpoint = `${API_URL}/reports/inventory-reports/export-pdf`;
                    ...
                    data?.data.map((item) => (
                      <tr className="border-b" key={item.id}>
                        ...
                        {item.variations.length > 0 && (
                          <td className="py-3 px-6 text-sm whitespace-nowrap">
                            <Link
                              href={`/merchants/reports/sales/variations-sales/${item.id}`}
                            >
                              <Button
                                variant="outline"
