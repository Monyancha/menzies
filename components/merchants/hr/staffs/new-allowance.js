...
import {
  submitNewAllowance,
  fetchAllAllowances
} from "../../../../src/store/merchants/inventory/units-slice";
import store from "../../../../src/store/Store";
import Link from "next/link";

function NewAllowanceModal() {
  ...
  async function submitDetails() {
    ...
    try {
      await dispatch(submitNewAllowance(params)).unwrap();
      showNotification({
        position: "top-right",
        zIndex: 2077,
        title: "Success",
        message: "New Allowance Successfully Created",
        color: "green",
      });
      clearForm();
      setOpened(false);
      ...
      pars["branch_id"] = "All";
      store.dispatch(fetchAllAllowances(pars));
    } catch (e) {
      let message = null;
      if (e?.message ?? null) {
        message = e.message;
...
export default NewAllowanceModal;
