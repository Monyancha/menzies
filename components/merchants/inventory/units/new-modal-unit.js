...
import { IconUser,IconEy } from "@tabler/icons";
...
function NewModalUnit() {
  ...
  const unitFamiliesList = useSelector(
    (state) => state.branches.unitFamiliesList
  );
  ...
  async function submitDetails() {
    ...
    params["unit_family_id"] = 2;

    try {
      await dispatch(submitUnit(params)).unwrap();
      showNotification({
        position: "top-right",
        zIndex: 2077,
        title: "Success",
        message: "New Unit Successfully Created",
        color: "green",
      });
      clearForm();
      setOpened(false);
    ...
    <>
      ...
      >
        <section className="flex flex-col space-y-2 bg-light p-3 rounded-lg">
          <span className="text-dark text-sm font-bold">New Unit</span>
          {/* <TextInput
            placeholder="Name"
            label="Name"
            withAsterisk
