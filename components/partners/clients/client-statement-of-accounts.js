...
import { Table, TDateFilter } from "../../ui/layouts/scrolling-table";
import { downloadFile } from "../../../lib/shared/printing-helpers";
import {
  TheadDark,
  TrowDark,
} from "../../ui/layouts/scrolling-table-dark";
import StatelessLoadingSpinnerDark from "../../ui/utils/stateless-loading-spinner-dark";
import CardDark from "../../ui/layouts/card-dark";
import { fetchClientSoA } from "../../../src/store/partners/clients-slice";

export default function ClientStatementOfAccounts({ clientId }) {
  ...
  const records = useSelector((state) => state.clients.clientSoA);

  const isLoading = useSelector(
    (state) => state.clients.clientSoAStatus === "loading"
  );
  ...
  useEffect(() => {
    ...
    store.dispatch(fetchClientSoA(params));
  }, [accessToken, clientId, startDate, endDate, branch_id]);
  ...
  async function downloadPdf() {
    ...
    let url = `/partners/clients/${clientId}/statement_of_accounts_pdf?`;
