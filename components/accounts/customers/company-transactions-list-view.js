...
import { fetchCompanyTransactions } from "@/store/merchants/accounts/acounts-slice";
...
function CompanyTransactionListView({
  isCredited = false,
  companyId = null,
} = {}) {
  ...
  const isLoading = useSelector(
    (state) => state.accounts.companyTransactionsStatus === "loading"
  );
  const records = useSelector((state) => state.accounts.companyTransactions);
  ...
  useEffect(() => {
    ...
    store.dispatch(fetchCompanyTransactions(params));
  }, [accessToken, companyId, startDate, endDate, branch_id, isCredited]);
...
export default CompanyTransactionListView;
