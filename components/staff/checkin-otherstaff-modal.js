...
function CheckinOtherStaffModal({item}) {
  ...
  const  handleSubmit = async (e) => {
    ...
    const companyId = item.visits[0].company;
    ...
    try {
      ...
      const endpoint = `${API_URL}/checkin-otherstaff`;
      ...
      if (response.status === 200) {
        ...
        setLoading(false);
        setOpened(false);
        router.push('/visitors/otherstaffin')
      } else {
        showNotification({
          title: "Error",
...
export default CheckinOtherStaffModal;
